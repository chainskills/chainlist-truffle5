...
contract("ChainList", accounts => {
    ...
    it("should let us sell a first article", async () => {
        const receipt = await chainListInstance.sellArticle(
                articleName,
                articleDescription,
                web3.utils.toWei(web3.utils.toBN(articlePrice).toString(), "ether"), {from: seller});
        const article = await chainListInstance.getArticle();
        ...
        assert.equal(web3.utils.toBN(article._price), web3.utils.toWei(web3.utils.toBN(articlePrice).toString(), "ether"), "article price must be " + web3.utils.toWei(web3.utils.toBN(articlePrice).toString(), "ether"));
    });
})